...
The code below assumes you have all three packages installed, plus the `dplyr` package for some data wrangling and `StratPal` for generating synthetic proxy records.

### Workshop structure

This part of the workshop is a practical, meaning the participants work through the materials on their own pace, and the trainers are available for questions and discussion. You can work through the materials in any order, so feel free to start with a package that seems most relevant to you. To familiarize yourself with the syntax and handling of the packages, a list of tasks is provided at the end of each section. They are not mandatory, but feel free to work through them to deepen your understanding of the functionality.

20 minutes before workshop end we will come together to discuss insights you had and problems you ran into. Specifically, this part will focus on

1.  What model assumptions are justified for your specific time scale and depositional environment
2.  What functionality age-depth modeling software should have for you
...
-   `positionThickness`: stratigraphic uncertainty of positions as half thickness \* `distType` : `U` or `G` for a uniform distribution or a normal distribution
...
Now we hand these estimated median ages into the `admtools` package:
...
```{R}
proxy_record_height = time_to_strat(obj = proxy_record, x = adm)
...
plot(x = new_positions,
     y = abs(sed_rate_l(adm, new_positions)),
     type = "l",
     xlab = "Height [m]",
...
```{r}
#| results: hide
age_predictions2 = agePredict(model = age_model, 
...
`astroBayes` is an R package for combining astrochronology and radioisotopic geochronology published by @trayler2023. It creates age-depth models by simultaneously matching a proxy signal to targe frequencies and absolute radiometric dates.
...
Now we can estimate the age-depth model:
...
```{R}
adm = tp_to_adm(t = predictions$CI$median,
                h = predictions$CI$position,
                L_unit = "m",
                T_unit = "Myr")
plot(adm)
T_axis_lab(label = "Age")
L_axis_lab(label = "Depth")
...
cyc_signal = list(h = cyclostrat$position,
                  y = cyclostrat$value)
cyc_signal_time = strat_to_time(cyc_signal, adm)
...
First, we load the requried data from @dasilva2020
...
Here, we foucs only on the case with precession amplitude modulation - for shore eccentricity amplitude modulation see @hohmann_nonparametric_2024. We start by estimating the sedimentation rates using eTimeOpt
...
This already indicates that something interesting is happening: If 3He accumulation is constant in time, the peaks in observed tracer flux correspond to stratigraphic condensation (low sedimentation rates).
...
This age-depth model can be plotted by calling the standard `plot` function from R.
...
```{R}
plot(h_est, sed_rate_l(adm_m, h_est) * 100,
     xlab = "Time [kyr]",
...
Note the low sedimentation rate at the base of the clay layer (around 306.8 m) which coincides with the peak of the PETM.
...
```{R}
proxy_record_height = time_to_strat(proxy_record, adm_m)
plot(proxy_record_height,
     type = "l",
     xlab = "Proxy value",
     ylab = "Height [m]")
...
```{R}
recovery_times = get_time(adm, h = rev(c(base_recovery, recovery_3_top)))
